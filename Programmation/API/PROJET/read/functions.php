...
function GetProjetById($id){
  $req = "SELECT `PROJET_ID`, `FK_CLIENT_ID`, `PROJET_NOM`, `PROJET_DESCRIPTION`, `PROJET_VALIDER`, `PROJET_COMMENTAIRE` FROM `PROJET` WHERE `PROJET_ID` = :id";
...
function GetProjetcsForClient($id){
  $req = "SELECT `PROJET_ID`, `PROJET_NOM`, `PROJET_DESCRIPTION`, `PROJET_VALIDER`, `PROJET_COMMENTAIRE` FROM `PROJET` WHERE `FK_PERSONNE_ID` = :id";
  $query = database()->prepare($req);
  $query->bindParam(':id', $id, PDO::PARAM_STR);
  $query->execute();
  $matches = $query->fetchAll(PDO::FETCH_ASSOC);
